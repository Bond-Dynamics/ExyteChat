...
protocol SwipeActionable {
    ...
    var action: (any MessageProtocol, @escaping (any MessageProtocol, DefaultMessageMenuAction) -> Void) -> Void { get }
    var activeFor: (any MessageProtocol) -> Bool { get }
...
public struct SwipeAction<V: View>: SwipeActionable {
    let action: (any MessageProtocol, @escaping (any MessageProtocol, DefaultMessageMenuAction) -> Void) -> Void
    let activeFor: (any MessageProtocol) -> Bool
    let content: () -> V
    let background: Color?
    
    public init(@ViewBuilder content: @escaping () -> V, background: Color? = nil, activeFor: @escaping (any MessageProtocol) -> Bool = { _ in true}, action: @escaping (any MessageProtocol, @escaping (any MessageProtocol, DefaultMessageMenuAction) -> Void) -> Void) {
    ...
    public init(action: @escaping (any MessageProtocol, @escaping (any MessageProtocol, DefaultMessageMenuAction) -> Void) -> Void, activeFor: @escaping (any MessageProtocol) -> Bool = { _ in true}, background: Color? = nil, @ViewBuilder content: @escaping () -> V) {
        self.content = content
        self.action = action
        self.activeFor = activeFor
        self.background = background
